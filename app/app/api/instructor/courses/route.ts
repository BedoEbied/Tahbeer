...
import { createCourseSchema, updateCourseSchema, courseIdSchema } from '@/lib/validators/course';
import { ApiResponse, UserRole, ICourse, UpdateCourseDTO } from '@/types';
import { ZodError } from 'zod';

/**
 * GET /api/instructor/courses - Get instructor's own courses
 */
async function getInstructorCoursesHandler(req: NextRequest, context: { user: any; params: Promise<{}> }) {
  try {
 ...
 */
async function createInstructorCourseHandler(req: NextRequest, context: { user: any; params: Promise<{}> }) {
  try {
